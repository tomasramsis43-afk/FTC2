...
// مجلد الحزمة الثابتة (حيث يُخزَّن app-assets وconfig.json بجانب المثبَّت) —
// هذا المتحكم يُسمح له بالتنزيل من الريبو والعمل من مجلد بيانات المستخدم، لذا
// يكون مساره مختلفاً عن مجلد الحزمة، ونحدّده هنا عبر متغير يُحقنه الـ bootstrap
// عند الإقلاع (fallback إلى __dirname في وضع التطوير حيث بجانب المتحكم مباشرة).
const BUNDLE_DIR = process.env.FTC2_BUNDLE_DIR || __dirname;
...
try {
  const cfgPath = require('path').join(BUNDLE_DIR, 'config.json');
  ...
  return new Promise((resolve, reject) => {
    ...
    async function arkkanAgentPath() {
      const candidates = [
        ...
        path.join(BUNDLE_DIR, '..', 'arkkan-agent.js'),     // وضع التطوير (جذر المشروع)
    ...
    srv.use(express.static(path.join(BUNDLE_DIR, 'app-assets'), { etag: false, lastModified: false, cacheControl: false }));
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    icon: path.join(BUNDLE_DIR, 'app-assets', 'icons', 'icon-512.png'),
    webPreferences: {
