...
function fillThemeColorInputs(){
  const c = Object.assign({}, DEFAULT_SETTINGS.themeColors, settings.themeColors);
  if($('#tc-navy')) $('#tc-navy').value = c.navy;
  if($('#tc-navy-dark')) $('#tc-navy-dark').value = c.navyDark;
  if($('#tc-gold')) $('#tc-gold').value = c.gold;
  if($('#tc-gold-dark')) $('#tc-gold-dark').value = c.goldDark;
  if($('#tc-gold-soft')) $('#tc-gold-soft').value = c.goldSoft;
...
if($('#btn-save-theme-colors')) $('#btn-save-theme-colors').addEventListener('click', async ()=>{
  settings.themeColors = {
    navy: $('#tc-navy').value,
    navyDark: $('#tc-navy-dark').value,
    gold: $('#tc-gold').value,
    goldDark: $('#tc-gold-dark').value,
    goldSoft: $('#tc-gold-soft').value,
    teal: $('#tc-teal').value,
    red: $('#tc-red').value
  };
  applyThemeColors(settings.themeColors);
  await saveSettings();
  await logAudit('edit','الإعدادات','تم تخصيص ألوان واجهة البرنامج');
  showToast('تم حفظ الألوان بنجاح');
});
if($('#btn-reset-theme-colors')) $('#btn-reset-theme-colors').addEventListener('click', async ()=>{
  settings.themeColors = JSON.parse(JSON.stringify(DEFAULT_SETTINGS.themeColors));
  fillThemeColorInputs();
  applyThemeColors(settings.themeColors);
  await saveSettings();
