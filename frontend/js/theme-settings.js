const DEFAULT_SETTINGS = {
  ...
  // بكود السيرفر التي كانت تفرض قيداً مختلفاً وأوسع لدور 'استقبال'). المرجع الفعلي الآن جدول
...
let bagStock = [];
let vaultTx = [];
let vaultTxVersion = 0; // يزداد مع كل تعديل لمصفوفة الحركات — للمemoization وتجنب إعادة الحساب الثقيل
...
let followUpTasks = []; // مهام/تذكيرات المتابعة اليدوية (مركز المتابعة والتذكيرات بلوحة التحكم) — كل مهمة: {id, text, dueDate, clientId, clientName, status:'open'|'done', createdBy, createdAt, doneAt}. مستقلة تماماً عن التنبيهات الذكية التلقائية (renderSmartAlerts)
...
const I18N = {
  ar: {
    ...
    noCourseNumberCol:'بدون رقم دورة',
    noCourseNumberTitle:'وسّم هذا العميل كعميل لن يُحدَّد له رقم دورة — يُستبعد من هذه القائمة، ومن مزامنة أركان، ومن شيت فواتير الدورات',
    noCourseNumberBoxTitle:'عملاء بدون رقم دورة (موسومون)',
    noCourseNumberBoxHint:'هؤلاء العملاء وُسموا يدوياً بـ"بدون رقم دورة" — مستبعدون من قائمة "من لم يحدد له دورة"، ومن مزامنة أركان، ومن شيت فواتير الدورات. ألغِ التوسيم لإرجاع أي عميل لحالته الطبيعية.',
    noCourseNumberEmptyMsg:'لا يوجد عملاء موسومون بـ"بدون رقم دورة" حالياً',
    unmarkBtnLabel:'إلغاء التوسيم',
    ...
    unknownCourseType:'⚠ الدورات غير المعلومة',
  ...
  en: {
    ...
    noCourseNumberCol:'No Course Number',
    noCourseNumberTitle:'Mark this client as one who will not be assigned a course number — excluded from this list, from Arkkan sync, and from the course invoices sheet',
    noCourseNumberBoxTitle:'Clients without a course number (marked)',
    noCourseNumberBoxHint:'These clients were manually marked "no course number" — excluded from the unassigned-course list, from Arkkan sync, and from the course invoices sheet. Unmark to return a client to normal status.',
    noCourseNumberEmptyMsg:'No clients are currently marked "no course number"',
    unmarkBtnLabel:'Unmark',
    ...
    unknownCourseType:'⚠ Unknown course type',
...
if($('#btn-toggle-server-sync')) $('#btn-toggle-server-sync').addEventListener('click', async ()=>{
  if(!manualOfflineMode){
    if(!await customConfirm('سيتوقف البرنامج عن الاتصال بالسيرفر تماماً، وسيعمل فقط بالنسخة المحفوظة على هذا الجهاز. أي تعديل تُجريه سيُحفظ محلياً بدون رفعه للسيرفر حتى تُعيد تفعيل الاتصال يدوياً. متابعة؟')) return;
