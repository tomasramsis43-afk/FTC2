...
/* ترحيل رجعي (Backfill) لحركات "صادر" القديمة بتصنيف "حقائب" التي لم تُرحَّل من قبل لحساب/مخزون
   الحقائب — سواء لأنها سُجّلت قبل إضافة هذه الأتمتة، أو لأن خانة التفعيل اليدوية القديمة لم تُفعَّل
   وقتها. للأدمن فقط. يعمل على مرحلتين: فحص (dry-run) لعرض العدد والمبلغ الإجمالي قبل أي تعديل فعلي،
   ثم التنفيذ الفعلي بعد تأكيد المستخدم.
   طريقة الكشف عن الحركات المُرحَّلة مسبقاً:
   1) أي حركة صادر لها بالفعل سجل في bagStock بحقل sourceVaultTxId يساوي معرّفها (الآلية الحالية).
   2) أي حركة صادر قديمة (من قبل إضافة sourceVaultTxId) — تُقارَن بسجلات bagStock من نوع "إيداع" التي
      نصّها يحتوي "مرتبط تلقائياً بحركة مالية" ولا تحمل sourceVaultTxId، بمطابقة (التاريخ + المبلغ) كزوج
      فريد لكل سجل (multiset)، لأن لا رابط مباشر (ID) محفوظاً بينهما في تلك الحقبة القديمة. هذه مطابقة
      تقريبية أفضل ما هو متاح دون رابط صريح — أي حركتين قديمتين لهما نفس التاريخ والمبلغ بالضبط ستُطابَقان
      كزوج واحد بغض النظر عن أيهما الأصلي فعلياً، وهذا لا يؤثر على صحة الرصيد الإجمالي بأي حال. */
function computeBagDepositBackfillCandidates(){
  const coveredIds = new Set(bagStock.filter(b=>b.sourceVaultTxId).map(b=>b.sourceVaultTxId));
  const legacyCounter = {};
  bagStock.forEach(b=>{
    if(b.sourceVaultTxId) return;
    if(b.type!=='deposit') return;
    if(!String(b.notes||'').includes('مرتبط تلقائياً بحركة مالية')) return;
    const key = `${b.date}|${num(b.amount)}`;
    legacyCounter[key] = (legacyCounter[key]||0) + 1;
  });
  const candidates = [];
  vaultTx.forEach(t=>{
    if(t.type!=='out' || t.isReturn) return;
    if(!String(t.category||'').includes('حقائب')) return;
    if(coveredIds.has(t.id)) return;
    const key = `${t.date}|${num(t.amount)}`;
    if(legacyCounter[key] > 0){ legacyCounter[key]--; return; } // مُرحَّلة مسبقاً على الأرجح
    candidates.push(t);
  });
  return candidates;
}
async function backfillBagDepositsFromOldExpenses(){
  if(currentUserRole!=='admin'){ showToast('هذه العملية للأدمن فقط'); return; }
  const candidates = computeBagDepositBackfillCandidates();
  if(!candidates.length){ showToast('لا توجد حركات مصروف بتصنيف "حقائب" تحتاج ترحيلاً — كل الحركات مُرحَّلة بالفعل'); return; }
  const total = candidates.reduce((s,t)=>s+num(t.amount),0);
  const ok = await customConfirm(
    `تم العثور على ${candidates.length} حركة صادر بتصنيف "حقائب" لم تُرحَّل بعد لحساب/مخزون الحقائب، بإجمالي ${fmt(total)} ﷼. سيُضاف كل مبلغ لرصيد حساب الحقائب وتُحتسب الحقائب الناتجة تلقائياً حسب السعر الثابت الحالي. هل تريد المتابعة؟`,
    'ترحيل حركات الحقائب القديمة'
  );
  if(!ok) return;
  snapshotState(`ترحيل رجعي لحركات صادر قديمة بتصنيف حقائب إلى حساب الحقائب (${candidates.length} حركة)`);
  let migrated = 0;
  candidates.forEach(t=>{
    bagStock.push({
      ...
      amount: num(t.amount),
      method: t.method,
      notes: `ترحيل رجعي لحركة مصروف قديمة بتصنيف حقائب (رقم تسلسلي #${t.seq||'—'})${t.notes ? ' — '+t.notes : ''}`,
      sourceVaultTxId: t.id
    });
    migrated++;
  });
  recalcBagFundLedger();
  await saveBagStock();
  await saveSettings();
  await logAudit('add','مخزون الحقائب', `ترحيل رجعي: تمت إضافة ${migrated} حركة إيداع لحساب الحقائب من حركات صادر قديمة بتصنيف "حقائب" (إجمالي ${fmt(total)} ﷼، الرصيد المتبقي: ${fmt(settings.bagFundBalance)})`);
  renderBags(); renderReports();
  showToast(`تم ترحيل ${migrated} حركة قديمة بنجاح ✅`);
}
$('#btn-backfill-bag-deposits')?.addEventListener('click', backfillBagDepositsFromOldExpenses);
