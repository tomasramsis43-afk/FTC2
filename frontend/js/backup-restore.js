...
async function restoreFullBackup(file){
  ...
  await pushCurrentDataToServer().catch(e=>{
    ...
    showToast('تعذّر رفع البيانات إلى السيرفر حالياً — ستُحفَظ محلياً وتُرفع تلقائياً عند استقرار الاتصال');
  });
...
async function restoreBagStockOnly(file){
  ...
  try{
    ...
    if(verified){
      ...
      showToast('اكتملت الاستعادة محلياً، لكن التحقق من السيرفر لم يكتمل — ستُرفع تلقائياً عند استقرار الاتصال');
    }
  }catch(e){
