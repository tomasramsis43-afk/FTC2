...
async function renderTable(){
  ...
  if(clientsQueryIsSimple() && Number.isFinite(pageSize)){
    ...
    try{
      ...
      // من كل البيانات رغم عدم وجود فلتر — فنكمل فوراً بالمسار المحلي الكامل أدناه حيث تكون كل
      // بيانات الذاكرة حاضرة دائماً. هذا يضمن أن "بدون فلتر" = كل العملاء فعلاً.
      if(!q && !fc && !fn && !dfrom && !dto && Number(data.total) !== clients.length){
        throw new Error('server total mismatch vs local');
      }
      // نفس فكرة الفحص أعلاه لكن مع وجود فلتر (نوع دورة/جنسية/تاريخ): كان يُغطّى فقط حالة "بلا
      // فلتر إطلاقاً" وحالة "نتيجة صفر" أدناه، فيفلت أي تركيبة فلاتر ترجع من السيرفر عدداً أقل من
      // الصحيح لكن غير صفري (مثال حقيقي: فلترة سنة + نوع دورة معاً رجّعت من السيرفر 45 بينما
      // المسار المحلي — ونفس أرقام المدفوع/المتبقي المعروضة أصلاً بالمعتمدة عليه — يطابق عدداً أكبر
      // بكثير)، فتظهر شاشة العملاء متسقة داخلياً (لأن كل شيء فيها من نفس رد السيرفر) لكن غير متسقة
      // مع بقية الشاشة (كروت فلتر الدورات، إجمالي المدفوع/المتبقي) المحسوبة من المسار المحلي الكامل.
      // نقارن هنا بعدد المطابقين فعلياً محلياً (البيانات محمّلة أصلاً بالذاكرة فلا كلفة شبكة إضافية)،
      // وأي فرق يعني نفس خلل تطابق clients_rows، فنتجاهل رد السيرفر ونكمل بالمسار المحلي الدقيق.
      if(!q && (fc || fn || dfrom || dto) && Number(data.total) !== filteredClients().length){
        throw new Error('server filtered total mismatch vs local');
      }
      if(data.total === 0){
