  ...
  $('#vault-table-body').innerHTML = pageRows.map(t=>{
    ...
    <tr>
      ...
      <td class="card-full" data-label="" style="white-space:nowrap;">
        ${(t.type==='in' && t.autoClientId) ? `<span class="hint" style="margin:0; display:inline-block; font-size:11px;">🔗 دفعة تسجيل — التعديل من شيت العملاء</span> ${((t.destination||'vault')==='vault') ? `<button class="btn btn-gold btn-sm" data-vreceipt="${t.id}" title="طباعة سند قبض">🧾 سند قبض</button>` : ''}${vApproveBtns}` : (t.type==='in' && t.companyTransferId) ? `
