...
let SERVER_AUTH_ROLE = null; // صلاحية المستخدم (admin/staff) كما أرجعها الخادم — هي المرجع الوحيد للصلاحيات الآن
...
async function serverLogin(username, password){
  ...
  SERVER_AUTH_ROLE = data.role === 'admin' ? 'admin' : 'staff';
  try{
...
/* ---------------- Nav ---------------- */
const RESTRICTED_STAFF_VIEWS = ['settings','audit','accounting','zatca'];
$all('nav.tabs button').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    if(currentUserRole==='staff' && RESTRICTED_STAFF_VIEWS.includes(btn.dataset.view)){
      showToast('هذا القسم متاح للمدير فقط');
      return;
    }
    SoundFX.nav();
...
/* إظهار/إخفاء التبويبات المحصورة على المدير فقط حسب صلاحية المستخدم الحالي */
function applyRolePermissions(){
  const isStaff = currentUserRole==='staff';
  $all('nav.tabs button').forEach(btn=>{
    btn.style.display = (isStaff && RESTRICTED_STAFF_VIEWS.includes(btn.dataset.view)) ? 'none' : '';
  });
  // إن كان المستخدم على أحد الأقسام المحظورة (مثلاً بعد تسجيل دخول مستخدم آخر بنفس الجلسة) نعيده للوحة التحكم
  if(isStaff){
    const activeBtn = $('nav.tabs button.active');
    if(activeBtn && RESTRICTED_STAFF_VIEWS.includes(activeBtn.dataset.view)){
...
function openModal(id){
  ...
  $('#f-bagsource').value = c?.bagSource || 'stock';
...
$('#btn-bulk-delete').addEventListener('click', openBulkDeleteModal);
...
function renderUsersList(){
  $('#users-list').innerHTML = users.map((u,i)=>`
    <div class="tag" style="border-radius:8px; justify-content:space-between; width:100%; margin-bottom:6px;">
      <span>👤 ${escapeHtml(u.username)}${u.username===currentUser ? ' (أنت)' : ''}
        <span class="mono" style="font-size:10.5px; color:${u.role==='staff'?'var(--text-muted)':'var(--gold-dark)'}; margin-right:6px;">${u.role==='staff' ? 'موظف' : 'مدير'}</span>
      </span>
      <button data-ru="${i}" ${users.length<=1 ? 'disabled title="لا يمكن حذف آخر مستخدم"' : ''}>✕</button>
    </div>`).join('');
}
...
$('#btn-add-user').addEventListener('click', async ()=>{
  ...
  const urole = $('#new-user-role').value === 'admin' ? 'admin' : 'staff';
  if(!uname || !upass){ showToast('أدخل اسم المستخدم وكلمة المرور'); return; }
  if(users.some(u=>u.username.toLowerCase()===uname.toLowerCase())){ showToast('اسم المستخدم موجود بالفعل'); return; }
  snapshotState(`إضافة مستخدم: ${uname}`);
  users.push({username:uname, password:upass, role:urole, createdAt:Date.now()});
  await saveUsers();
  await logAudit('add','المستخدمون', `تمت إضافة مستخدم جديد: ${uname} (${urole==='admin'?'مدير':'موظف'})`);
...
document.addEventListener('click', async e=>{
  if(e.target.dataset.ru!==undefined){
    const idx = +e.target.dataset.ru;
    if(users.length<=1) return;
    const removed = users[idx];
    if(await customConfirm(`تأكيد حذف المستخدم "${removed.username}"؟`)){
      snapshotState(`حذف مستخدم: ${removed.username}`);
      users.splice(idx,1);
      await saveUsers();
      await logAudit('delete','المستخدمون', `تم حذف المستخدم: ${removed.username}`);
      renderUsersList();
...
function autoSignInLocalUser(){
  ...
  currentUserRole = SERVER_AUTH_ROLE === 'admin' ? 'admin' : 'staff';
...
/* ---- الإقرار الضريبي (ضريبة القيمة المضافة): ضريبة المخرجات من فواتير الدورات، ضريبة المدخلات من المشتريات ---- */
function buildVatReturn(from, to){
  // ضريبة المخرجات: فواتير الدورات التي لها رقم فاتورة وقيمة فعلية بالإيصال، حسب تاريخ صدور الفاتورة
  const salesRows = courseInvoiceClients().filter(c=>{
    const d = c.receiptIssueDate || '';
    return num(c.receiptActualValue) > 0 && d && d>=from && d<=to;
  });
  const salesGross = salesRows.reduce((s,c)=> s+num(c.receiptActualValue), 0);
  const outputVat = salesRows.reduce((s,c)=> s+courseInvoiceVat(c.receiptActualValue), 0);
  const salesNet = salesGross - outputVat;
  ...
  salesRows.sort((a,b)=> String(a.receiptIssueDate||'').localeCompare(String(b.receiptIssueDate||'')));
  purchaseRows.sort((a,b)=> String(a.date||'').localeCompare(String(b.date||'')));
  return { salesRows, purchaseRows, salesGross, salesNet, outputVat, purchasesGross, purchasesNet, inputVat, netVat };
}
function renderVatReturnTable(from, to){
  ...
  table.innerHTML = `<tbody>
    ...
    ${row('إجمالي المبيعات بدون الضريبة', r.salesNet, {indent:true, muted:true})}
    ${row('إجمالي ضريبة المخرجات (15%)', r.outputVat, {bold:true})}
...
$('#btn-export-vat')?.addEventListener('click', ()=>{
  ...
  const summaryRows = [
    ...
    {'البند':'إجمالي المبيعات بدون الضريبة', 'القيمة':r.salesNet},
    {'البند':'إجمالي ضريبة المخرجات (15%)', 'القيمة':r.outputVat},
  ...
  const salesDetailRows = r.salesRows.map(c=>{
    ...
    return {
      'التاريخ': c.receiptIssueDate||'', 'العميل': c.name||'', 'رقم الهوية': c.clientId||'', 'رقم الفاتورة': c.invoice||'',
      'القيمة بدون الضريبة': num(c.receiptActualValue) - vat, 'الضريبة': vat, 'الإجمالي': num(c.receiptActualValue),
    };
  });
...
$('#btn-add-journal')?.addEventListener('click', async ()=>{
  ...
  journalEntries.push({ id: uid(), createdAt: Date.now(), type, date, amount, description });
  await saveJournalEntries();
  await logAudit('add','المحاسبة', `تمت إضافة قيد يدوي (${$('#jf-type').selectedOptions[0].textContent}): ${description} بمبلغ ${fmt(amount)} ﷼`);
  $('#jf-amount').value=''; $('#jf-desc').value='';
  showToast('تمت إضافة القيد');
  renderAccounting();
});
$('#acc-journal-body')?.addEventListener('click', async e=>{
  ...
  if(!j) return;
  if(!await customConfirm('هل تريد حذف هذا القيد اليدوي؟')) return;
...
/* جدولا تفاصيل الفواتير (مبيعات ثم مشتريات) لطباعة الإقرار الضريبي: رقم الفاتورة، تاريخ الفاتورة، الضريبة، القيمة بدون الضريبة فقط */
function buildVatDetailTablesHtml(r){
  const salesBody = r.salesRows.map(c=>{
    const vat = courseInvoiceVat(c.receiptActualValue);
    const net = num(c.receiptActualValue) - vat;
    const total = net + vat;
    return `<tr><td class="mono">${escapeHtml(c.invoice||'—')}</td><td class="mono">${escapeHtml(formatDateDisplay(c.receiptIssueDate)||'')}</td><td class="mono">${fmt(vat)}</td><td class="mono">${fmt(net)}</td><td class="mono">${fmt(total)}</td></tr>`;
  }).join('');
...
$('#btn-print-vat')?.addEventListener('click', ()=>{
  const { from, to } = accSelectedRange();
  const r = buildVatReturn(from, to);
  printAccountingReport('الإقرار الضريبي (ضريبة القيمة المضافة)', '#acc-vat-table', { extraHtml: buildVatDetailTablesHtml(r) });
});
...
function printCompanyStatement(companyId){
  ...
  const traineesRows = allTrainees.length ? allTrainees.map(({tr,t})=>{
    ...
    return `<tr>
      ...
      <td>${tr.posted ? 'تم الترحيل' : 'لم يُرحَّل'}</td>
    </tr>`;
  }).join('') : `<tr><td colspan="10" style="text-align:center; color:${PRINT_PALETTE.muted};">لا يوجد متدربون مضافون بعد</td></tr>`;
  ...
  <body>
    ...
    <table>
      <thead><tr><th>تاريخ الحوالة</th><th>رقم الهوية</th><th>الاسم</th><th>الجنسية</th><th>نوع الدورة</th><th>رقم الدورة</th><th>قيمة الدورة</th><th>قيمة الحقيبة</th><th>الإجمالي</th><th>حالة الترحيل</th></tr></thead>
...
$('#purchase-form')?.addEventListener('submit', async e=>{
  ...
  if(existing){
    ...
    purchases.push({id: uid(), supplierId, supplierName: supplier.name, invoiceNo, date, items, subtotal, taxAmount, total, method, status, notes, vaultTxId, attachment: currentPurchaseAttachment, createdAt: Date.now()});
    ...
    try{
      ...
      if(res.ok){
        try{
          ...
          SERVER_AUTH_ROLE = sessionStorage.getItem('serverAuthRole') === 'admin' ? 'admin' : 'staff';
