...
function buildSandbox(){
  ...
  const sandbox = {
    console,
    // ملاحظة مهمة: setInterval/setTimeout الحقيقية من Node ما تُمرَّرش هنا مباشرة عمداً.
    // core-utils.js (من ضمن الملفات التي تُحمَّل غالباً) يسجّل عدة setInterval حقيقية على
    // مستوى عام عند التحميل (مراقبة دورية لعناصر الجدول، إلخ) — لو استخدمنا مؤقّتات Node
    // الحقيقية هنا، كل اختبار يستدعي loadFrontendFiles() يُسرّب مؤقّتاً حياً جديداً يفضل
    // شغّالاً للأبد فى نفس الـ process، فيمنع node:test من الخروج بعد انتهاء كل الاختبارات
    // (العملية تظل حية بلا سبب وتُعلَّق npm test إلى الأبد). بما أن الدوال المُختبَرة هنا
    // (assertBalancedLines/allocVaultSeq/roundMoney/vatFromGross...) منطق حسابي متزامن بحت
    // ولا تعتمد فعلياً على تنفيذ أي مؤقّت، نستبدلها بمؤقّتات وهمية لا تُنفَّذ أبداً — تمنع
    // تسرّب المؤقّتات دون أي أثر على صحة الاختبارات نفسها.
    ...
    requestAnimationFrame: fn => 0, // نفس منطق setTimeout أعلاه: لا حاجة لتنفيذه فعلياً هنا
    Intl,
